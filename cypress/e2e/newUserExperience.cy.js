    ...
    cy.get(".card").each((el) => {
      cy.wrap(el).within(() => {
        cy.get("p")
          ...
          .then((price) => {
            expect(
              parseFloat(price.text().replace("Price: ", "")),
            ).to.be.within(100, 200);
          });
      });
    });
  });
...
describe("Visiting a chef's profile", () => {
  ...
  it("should place an order and submit the order form successfully (alice waters as example)", () => {
    ...
    cy.intercept(
      "POST",
      "http://localhost:8000/chefs/664fbcf07a311cad9a3e408a/order",
      {
        statusCode: 201,
        body: { success: true },
      },
  ...
  it("should make a review", () => {
    ...
    cy.intercept(
      "POST",
      "http://localhost:8000/chefs/664fbcf07a311cad9a3e4088/reviews",
      {
        statusCode: 201,
        body: { success: true },
      },
